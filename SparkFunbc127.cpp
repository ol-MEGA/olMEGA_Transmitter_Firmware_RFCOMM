...
BC127::opResult BC127::stdGetConfig(String *param)
{
  String buffer = "\n";
  String EOL = String("\r");

  knownStart();  // Clear the serial buffers.
  _serialPort->print("CONFIG");
  _serialPort->print("\r");
  _serialPort->flush();
  unsigned long loopStart = millis();
  while (loopStart + 2000 > millis() && !buffer.endsWith("OK"))
  {
    if (_serialPort->available() >0) buffer.concat(char(_serialPort->read()));
    if (buffer.endsWith(EOL))
      buffer.concat("\n");
...
BC127::opResult BC127::stdGetCommand(String command, String parameter, String *param)
{
  String buffer;
  String EOL = String("\r");
